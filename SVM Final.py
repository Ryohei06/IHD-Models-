...
features = ['Age', 'Diabetes', 'Stroke', 'HBP', 'HBC', 'Smoker']  # Assuming these are your features
X = data[features]  # Select features
y = data['HDA']  # Target variable

## Split data into training and testing sets
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25)  # Adjust test_size as needed

# Check target variable shape and prepare for SVM model *after* splitting
if len(y_train.shape) == 2 and y_train.shape[1] > 1:
    target_column_index = 1  # Assuming you want to use the second column
    y_train_svm = y_train.iloc[:, target_column_index]  # Extract specific column
else:
    y_train_svm = y_train.values  # Convert to 1D array if needed
...
precision = precision_score(y_test, y_pred, average='weighted',zero_division=1)  #how well does the model identify all positive cases (avoiding false negatives)
recall = recall_score(y_test, y_pred, average='weighted')  #measures the proportion of actual positive cases the model identified correctly.
f1 = f1_score(y_test, y_pred, average='weighted')  #considers both the model's ability to identify positive cases (recall) and avoid false positives (precision). good metric for imbalanced datasets
